...
export async function getSamhallsbyggeItems(
  options: SamhallsbyggeQueryOptions = {},
): Promise<{ items: SamhallsbyggeItem[]; error: string | null }> {
  try {
    const url = new URL("/api/samhallsbygge", getAppBaseUrl());
    if (options.area?.trim()) {
      url.searchParams.set("area", options.area.trim());
    }
