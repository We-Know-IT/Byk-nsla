...
const DEFAULT_CENTER: [number, number] = [13.191, 55.704];
...
const buildPopupHtml = (item: SamhallsbyggeItem): string => {
  ...
  return `
    <div class="samhallsbyggeTooltip" role="dialog" aria-label="Kartinformation">
      <p class="samhallsbyggeTooltipCategory">${category}</p>
      <h3 class="samhallsbyggeTooltipTitle">${title}</h3>
      <p class="samhallsbyggeTooltipMeta">Status: ${status}</p>
      ${reference ? `<p class="samhallsbyggeTooltipMeta">Ref: ${reference}</p>` : ""}
      ${subtitle ? `<p class="samhallsbyggeTooltipMeta">${subtitle}</p>` : ""}
    </div>
  `;
};
...
export default function SamhallsbyggeMap({ items }: SamhallsbyggeMapProps) {
    ...
    const syncMapData = () => {
      for (const item of mapItems) {
        ...
        if (center) {
          ...
          markerElement.className = "samhallsbyggeMarker";
        ...
        if (item.geometry.type === "Polygon" || item.geometry.type === "MultiPolygon") {
          ...
          const openPolygonPopup = (event: mapboxgl.MapMouseEvent) => {
  ...
  if (!token) {
    return (
      <div className="mapFallback samhallsbyggeMapFallback">
        <p>Karta visas här när NEXT_PUBLIC_MAPBOX_TOKEN finns i web/.env.</p>
      </div>
    );
  }

  return <div ref={containerRef} className="mapCanvas samhallsbyggeMapCanvas" />;
}
