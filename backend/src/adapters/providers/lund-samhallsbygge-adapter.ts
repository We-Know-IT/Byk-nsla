...
async function fetchFeatureCollection(url: string): Promise<ArcGisFeature[]> {
...
export class LundSamhallsbyggeAdapter implements SamhallsbyggeAdapter {
  async getItems(): Promise<ExternalSamhallsbyggeItem[]> {
    const [kungorelseFeatures, grannhorandeFeatures, detaljplanFeatures] = await Promise.all([
      fetchFeatureCollection(BYGGLOV_KUNGORELSE_URL),
      fetchFeatureCollection(GRANNHORANDE_URL),
      fetchFeatureCollection(DETALJPLAN_URL),
    ]);
